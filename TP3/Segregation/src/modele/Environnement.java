...
public abstract class Environnement extends Observable {
     ...
     * Retire un agent des agents presents dans l'environnement
     *
     * @param i : l'indice de l'agent a retirer
    ...
    /**
     * Calcul la valeur de la satisfaction de tout les agents
     *
     * @return la satisfaction moyenne dans l'environnement
     */
    public int getGlobalSatisfaction() {
        int size = agents.size();
        int moy = 0;
        for (int i = 0; i < size; i++) {
            moy += agents.get(i).getSatisfaction();
        }
        return moy / size;
    }
}
