    ...
    public void action() {
        ...
        for (int l = getY()-1 ; l  <= getY()+1 ; l++){
            for (int c = getX()-1 ; c <= getX()+1 ; c++){
                if (((Grille)this.getEnvironnement()).isInGrid(c, l) && !hasMoved){                    
                    if (!((Grille)this.getEnvironnement()).isFree(c, l) && ((Grille)this.getEnvironnement()).getCreature(c, l).getType() == Type.POISSON){
                        //On mange le poisson
                        ((Grille)this.getEnvironnement()).removeCreature(((Grille)this.getEnvironnement()).getCreature(c, l));
        ...
        if(!hasMoved){
            ...
            while (!((Grille)this.getEnvironnement()).isInGrid(this.getX() +movX, this.getY()+movY) && !allUsed){
        ...
        if (getAge() % getTauxReproduction() == 0 ){
            ...
            if(!allUsed){
                ...
                ((Grille)getEnvironnement()).addCreature(new Requin(), this.getX()+movX, this.getY()+movY);
            }
        }
        age++;
        pv--;
